...
const AdminComments = () => {
    ...
    const fetchComments = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:6969/api/admin/comments?${params}`, {
    ...
    const handleStatusChange = async (commentId, newStatus) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`http://localhost:6969/api/admin/comments/${commentId}/status`, {
                status: newStatus
            }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDeleteComment = async (commentId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:6969/api/admin/comments/${commentId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
