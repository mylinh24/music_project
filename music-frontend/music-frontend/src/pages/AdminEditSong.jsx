...
const AdminEditSong = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [songRes, artistsRes, categoriesRes] = await Promise.all([
          axios.get(`http://localhost:6969/api/admin/songs/${id}`, { headers: { Authorization: `Bearer ${token}` } }),
        ...
        setFormData({
          ...
          lyrics: song.lyrics || '',
          artist_id: song.artist ? song.artist.id : '',
          category_id: song.category ? song.category.id : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:6969/api/admin/songs/${id}`, formData, {
